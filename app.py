...
# Check if we need to run the data generation
def check_and_generate_data():
    required_files = [
        os.path.join(OUTPUT_DIR, "processed_prescriptions.xlsx"),
        os.path.join(SCRIPT_DIR, "doctor_insights.xlsx")
    ]
    ...
    if not all_exist:
        with st.spinner("Generating prescription data (this may take a few minutes)..."):
            try:
                ...
                result = subprocess.run(
                    [sys.executable, os.path.join(SCRIPT_DIR, "generat_data.py")],
                    capture_output=True,
                    text=True
                )
