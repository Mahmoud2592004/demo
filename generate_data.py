...
RAW_IMAGES_DIR = os.path.join(SCRIPT_DIR, "raw_images")
...
def ensure_min_prescriptions(records_list, min_per_city=5):
    ...
    for r in records_list:
        has_image_path = r.get('image_path') and isinstance(r['image_path'], str)
        image_exists = has_image_path and os.path.exists(r['image_path'])
        has_drugs = r.get('drugs') and json.loads(r['drugs']) if r.get('drugs') else False
        if not has_image_path:
            print(f"Prescription {r.get('prescription_id', 'unknown')} skipped: No valid image_path")
        elif not image_exists:
            print(f"Prescription {r.get('prescription_id', 'unknown')} skipped: Image not found at {r['image_path']}")
        elif not has_drugs:
            print(f"Prescription {r.get('prescription_id', 'unknown')} skipped: No valid drugs")
        else:
            prescriptions_with_images_and_drugs.append(r)
    ...
    for gov, city in all_cities:
        ...
        if current_with_images_and_drugs < min_per_city:
            ...
            for _ in range(needed):
                ...
                dest_folder = os.path.join(ORGANIZED_IMAGES_DIR, gov_folder, city_folder)
...
def process_prescriptions(input_file, drug_db_file):
    print("Loading data...")
    try:
        ...
        # Create image to record mapping
        image_to_record = {}
        ...
        print(f"- Prescriptions skipped due to duplicate images: {duplicate_image_skipped}")
        ...
        if doctor_names:
            ...
            doctor_counts.to_excel(os.path.join(SCRIPT_DIR, "doctor_insights.xlsx"), index=False)
...
def save_data(df):
    ...
    output_path = os.path.join(OUTPUT_DIR, "processed_prescriptions.xlsx")
    with pd.ExcelWriter(output_path) as writer:
        ...
        doctor_insights_path = os.path.join(SCRIPT_DIR, "doctor_insights.xlsx")
        if os.path.exists(doctor_insights_path):
            doctor_df = pd.read_excel(doctor_insights_path)
            doctor_df.to_excel(writer, sheet_name="Doctors", index=False)

if __name__ == "__main__":
    INPUT_FILE = os.path.join(SCRIPT_DIR, "prescriptions.xlsx")
    DRUG_DB_FILE = os.path.join(SCRIPT_DIR, "search engine MedDigi.xlsx")
    ...
    if not processed_df.empty:
        ...
        - Output file: {os.path.join(OUTPUT_DIR, "processed_prescriptions.xlsx")}
        """)
    else:
